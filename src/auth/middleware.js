...
export async function authenticateRequest(req, res, next) {
    ...
    if (token == null) return res.send({statusCode: 401, message: 'unauthorized access'});

    const decryptedToken = await Token.verifyToken(token, SECRET_KEY);
    if (!decryptedToken) return res.send({statusCode: 401, message: 'token expired'});
    ...
    if(!userExist) return res.send({statusCode: 401, message: 'unauthorized access'});
    return next();
}